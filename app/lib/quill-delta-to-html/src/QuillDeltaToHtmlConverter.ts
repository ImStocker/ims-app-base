...
import type { IOpAttributeSanitizerOptions } from './OpAttributeSanitizer';
...
export class QuillDeltaToHtmlConverter {
  ...
  constructor(deltaOps: any[], options?: IQuillDeltaToHtmlConverterOptions) {
    ...
    let inlineStyles: IInlineStyles | undefined;
  ...
  getGroupedOps(): TDataGroup[] {
    ...
    let groupedOps = Grouper.reduceConsecutiveSameStyleBlocksToOne(
    ...
    const listNester = new ListNester();
  ...
  _renderListItem(li: ListItem): string {
    ...
    const liElementsHtml = this._renderInlines(li.item.ops, false);
    return (
      parts.openingTag +
      liElementsHtml +
  ...
  _renderTableCell(cell: TableCell): string {
    ...
    const parts = converter.getHtmlParts();
    const cellElementsHtml = this._renderInlines(cell.item.ops, false);
    return (
      makeStartTag('td', {
        key: 'data-row',
  ...
  _renderBlock(bop: DeltaInsertOp, ops: DeltaInsertOp[]) {
    const converter = new OpToHtmlConverter(bop, this.converterOptions);
    const htmlParts = converter.getHtmlParts();
    ...
    const inlines = ops.map((op) => this._renderInline(op, bop)).join('');
