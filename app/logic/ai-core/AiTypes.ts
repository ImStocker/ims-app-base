...
export type AiToolCallAction = { type: 'tool-call'; toolName: string; args: any; result?: { success: boolean; result?: any; error?: string } };
...
export type AiTurn = {
  id: string;
  sessionId: string;
  userMessage: string;         // the user's input that started this turn
  actions: AiAction[];         // all actions emitted during generation (thinking, tool-calls, text)
  changeIds: string[];         // change IDs produced in this turn
  createdAt: string;           // when the turn was started
  completedAt?: string;        // when generation finished
