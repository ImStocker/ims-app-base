...
async function fetchOpenRouterModels(settings: Partial<AiSettingsModel>): Promise<{ name: string }[]> {
...
async function fetchOllamaModels(settings: Partial<AiSettingsModel>): Promise<{ name: string }[]> {
...
async function fetchOpenAiModels(settings: Partial<AiSettingsModel>): Promise<{ name: string }[]> {
...
async function fetchDeepseekModels(settings: Partial<AiSettingsModel>): Promise<{ name: string }[]> {
...
async function fetchGeminiModels(settings: Partial<AiSettingsModel>): Promise<{ name: string }[]> {
...
async function fetchGrokModels(settings: Partial<AiSettingsModel>): Promise<{ name: string }[]> {
...
export const AiModelDescriptorList: AiModelDescriptor[] = [
  {
    ...
    title: 'OpenRouter',
    properties: [
      { name: 'apiKey', placeholder: 'sk-or-', type: 'password' }
    ],
    fetchModels: {
      fetch: fetchOpenRouterModels,
      requireApiKey: true
    },
    note: 'sfAi.notes.openrouter'
  },
  {
    name: 'ollama',
    title: 'Ollama',
    properties: [
      { name: 'baseUrl', placeholder: 'http://localhost:11434', default: 'http://localhost:11434', type: 'text' },
    ],
    fetchModels: {
      fetch: fetchOllamaModels,
      requireApiKey: false,
      tags: [
        { name: 'gemma4:31b-cloud', description: 'sfAi.tags.gemma4' },
        { name: 'minimax-m3:cloud' }
      ]
    },
    note: 'sfAi.notes.ollama'
  },
  {
    name: 'gemini',
    title: 'Gemini',
    ...
    note: 'sfAi.notes.gemini'
  },
  {
    name: 'openai',
    title: 'OpenAI (GPT)',
    properties: [
      { name: 'apiKey', placeholder: 'sk-', type: 'password' },
    ],
    fetchModels: {
