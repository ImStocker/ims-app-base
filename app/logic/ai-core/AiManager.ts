...
import { type AiSettings, type AiSettingsModel } from './AiSettings';
...
export default class AiManager extends AppSubManagerBase {
  ...
  getAiModelDef(): AiModelDef | null {
    ...
    const model_params = settings.models.find(m => m.name === settings.selectedModel);
    if (!model_params) return null;
    if (!model_params.model) return null;

    const descriptor = AiModelDescriptorList.find(d => d.name === model_params.name)
  ...
  aiModelSettingsValidModel(): AiSettingsModel | null {
    ...
    const model = settings.models.find(m => m.name === settings.selectedModel);
    if (!model) {
      return null;
    }
    return model;
  ...
  addModel(model: AiSettingsModel): void {
    const settings = this.getSettings();
    const existingIndex = settings.models.findIndex(m => m.name === model.name);
    if (existingIndex >= 0) {
  ...
  getAiModel(): LanguageModel | null {
    ...
    switch (model_params.name) {
      case 'ollama':
        return createOllama({ baseURL: model_params.baseUrl })(model_params.model, {
          think: true,
        })
      case 'ollama_cloud':
        return createOllama({
      ...
      case 'openai':
        return createOpenAI({ apiKey: model_params.apiKey }).chat(model_params.model)
      case 'openrouter':
        return createOpenAI({
      ...
      case 'gemini':
        return createGoogleGenerativeAI({ apiKey: model_params.apiKey })(model_params.model)
      case 'deepseek':
        return createDeepSeek({ apiKey: model_params.apiKey })(model_params.model)
      case 'anthropic':
        return createAnthropic({ apiKey: model_params.apiKey })(model_params.model)
      case 'custom':
        return createOpenAI({
