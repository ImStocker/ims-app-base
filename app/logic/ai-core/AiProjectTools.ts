...
export type ListAssetsResult = {
  total: number;
  assets: Pick<AssetShort, 'id' | 'title' | 'name' | 'workspaceId' | 'typeIds'>[];
};
...
export type ListWorkspacesResult = {
  total: number;
  workspaces: Pick<Workspace, 'id' | 'title' | 'name' | 'parentId'>[];
};
...
export class AiProjectTools {
  ...
  get create_asset(): AiToolDef {
    return this._createAsset;
  }
  get change_asset(): AiToolDef {
    return this._changeAsset;
  }
  get delete_asset(): AiToolDef {
    return this._deleteAsset;
  }
  get get_asset(): AiToolDef {
    return this._getAsset;
  }
  get list_assets(): AiToolDef {
    return this._listAssets;
  }
  get search_assets(): AiToolDef {
    return this._searchAssets;
  }
  get create_workspace(): AiToolDef {
    return this._createWorkspace;
  }
  get list_workspaces(): AiToolDef {
    ...
    return {
      ...
      handler: async (args) => {
        ...
        try {
          ...
          return { success: true as const, deleted: result.ids.length > 0, changeId: result.changeId ?? null };
        } catch (err: unknown) {
          const msg = err instanceof Error ? err.message : String(err);
          return { success: false as const, error: msg };
        }
