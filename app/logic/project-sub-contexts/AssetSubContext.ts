...
import { TASK_COLUMNS_ID, BLOCK_NAME_META } from '#logic/constants';
import type { AssetFullInstanceR } from '#logic/types/AssetFullInstance';
import { AssetFullInstance } from '#logic/types/AssetFullInstance';
import type {
  AssetGlobalHistoryResultDTO,
  AssetHistoryDTO,
} from '#logic/types/AssetHistory';
import {
...
import type { AssetPropsPlainObject, AssetProps } from '#logic/types/Props';
import {
...
import Subscriber from '#logic/types/Subscriber';
import {
  type Workspace,
...
import { ProjectContentExternalListener } from './Asset/ProjectContentExternalListener';
import {
  ProjectSubContext,
  type IProjectContext,
} from '#logic/types/IProjectContext';
import ApiManager from '#logic/managers/ApiManager';
import { Service, HttpMethods } from '#logic/managers/ApiWorker';
...
export class AssetSubContext extends ProjectSubContext {
  ...
  private _fullAssetsCache: EntityCache<AssetFullInstanceR>;
  private _workspacesCache: EntityCache<Workspace>;
  private _externalContentListener: ProjectContentExternalListener | null =
    null;
  private _projectContentEvents = new Subscriber<
    [ProjectContentChangeEventArg]
  >();

  constructor(
    projectContext: IProjectContext,
    private readonly _projectDatabase: IProjectDatabase,
  ) {
  ...
  async init(watchExternalEvents: boolean) {
    if (watchExternalEvents && this.projectContext.user?.role) {
      ...
      this._externalContentListener.listenWorkpaceIds(
        this.projectContext.projectInfo.rootWorkspaces.map((w) => w.id),
      );
    }
  }
  ...
  get projectContentEvents() {
    return this._projectContentEvents;
  }

  //WORKSPACES

  private async _getWorkspacesListChunk(
  ...
  async getWorkspacesList(
    ...
    const res = await this._getWorkspacesListChunk(query);
    let has_more = (query.count ?? res.total) > res.list.length;
    let next_offset = (query.offset ?? 0) + res.list.length;
    while (has_more) {
      const chunk_query = {
        ...query,
        offset: next_offset,
      };
      const chunk = await this._getWorkspacesListChunk(chunk_query);
      if (chunk.list.length === 0) break;
      next_offset += chunk.list.length;
      res.list = [...res.list, ...chunk.list];
      has_more = (query.count ?? res.total) > res.list.length;
    }
    res.total = res.list.length;
    return res;
  }

  async loadWorkspaceById(workspace_id: string): Promise<Workspace | null> {
  ...
  async createWorkspace(
    ...
    if (params.props?.type === WORKSPACE_TYPE_COLLECTION) {
      const parent_workspace = params.parentId
        ? await this.loadWorkspaceById(params.parentId)
        : null;
      if (
        parent_workspace &&
        ...
        const new_view = {
          title: convertTranslatedTitle('[[t:Table]]', (key) =>
            this.projectContext.appManager.$t(key),
          ),
          type: ViewType.TABLE,
        };
      ...
      const task_columns_asset = await this.getAssetInstance(TASK_COLUMNS_ID);
    ...
    await this.projectContentEvents.notify({
      ...
      wDelIds: [],
      instigator: this.projectContext.user?.id ?? null,
    });
    return res;
  }
  ...
  updateFullInstancesCache(asset_fulls: AssetFull[], request_listen = true) {
    ...
    for (const asset of asset_fulls) {
      ...
      if (instance) {
        ...
        instance = AssetFullInstance.Create(this.projectContext, asset);
  ...
  private async _getAssetsViewChunk<T extends AssetPropsPlainObject>(
  ...
  async getAssetsView<T extends AssetPropsPlainObject>(
    ...
    const res = await (options
      ? this._getAssetsViewChunk<T>(query, options)
      : (this._getAssetsViewChunk<AssetProps>(query) as Promise<
    ...
    while (has_more) {
      ...
      const chunk = await (options
        ? this._getAssetsViewChunk<T>(chunk_query, options)
        : (this._getAssetsViewChunk<AssetProps>(chunk_query) as Promise<
            ApiResultListWithTotal<T>
          >));
      if (chunk.list.length === 0) break;
      next_offset += chunk.list.length;
  ...
  async changeAssets(params: AssetChangeDTO): Promise<AssetsChangeResult> {
    if (!params.set) {
      return {
        ...createEmptyAssetFullResult(),
        changeId: null,
  ...
  forgetAssetInCacheById(assetId: string) {
    ...
    this._hasChildrenAssetsCache.invalidateElement(assetId);
  }
  ...
  getParentWorkspacesListFromCacheSync(
    workspace_id: string,
  ): Workspace[] | undefined {
    const res: Workspace[] = [];
