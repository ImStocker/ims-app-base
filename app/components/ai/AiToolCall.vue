<template>
  <div class="AiToolCall" :class="{ 'is-done': action.result }">
    <div class="AiToolCall-header" @click="expanded = !expanded">
      ...
      <span v-if="action.result" class="AiToolCall-badge" :class="action.result.success ? 'success' : 'error'">
        {{ action.result.success ? '\u2713' : '\u2717' }}
      </span>
      <span v-else class="AiToolCall-badge pending">...</span>
      <i class="AiToolCall-chevron" :class="{ open: expanded }">\u25B8</i>
    </div>
    <div v-if="expanded" class="AiToolCall-body">
      <div class="AiToolCall-section">
        <div class="AiToolCall-sectionTitle">{{ t('aiAssistant.arguments') }}</div>
      ...
      <div v-if="action.result" class="AiToolCall-section">
        ...
        <pre v-if="action.result.success" class="AiToolCall-json">{{ formatJSON(action.result.result) }}</pre>
        <pre v-else class="AiToolCall-json error">{{ action.result.error }}</pre>
      </div>
    </div>
  </div>
</template>
...
import type { AiToolCallAction } from '~ims-app-base/logic/ai-core/AiTypes';
...
.AiToolCall-section {
  padding: 6px 10px;
  &:not(:last-child) { border-bottom: 1px solid var(--local-border-color, #333); }
}
